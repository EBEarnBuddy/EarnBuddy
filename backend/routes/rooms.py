...
from backend.models.rooms import RoomModel # Import RoomModel
...
@router.post("/room", response_model=RoomModel, status_code=status.HTTP_201_CREATED)
...
# --- Get All Rooms (e.g., public rooms or rooms user is a member of) ---
...
async def get_all_rooms(
    ...
    rooms = []
    # Fetch public rooms OR rooms where the current user is a member
    async for room_doc in db.rooms.find({
        "$or": [
            {"isPrivate": False},
...
# --- Get All Rooms (with optional search and topic filters) ---
@router.get("/", response_model=List[RoomModel], summary="Get all rooms with search and topic filters")
async def get_all_rooms(
    current_user_uid: str = Depends(get_current_user_uid),
    search_query: Optional[str] = None, # New: for search bar
    topics: Optional[List[str]] = Query(None) # New: for topic filtering (e.g., ?topics=AI/ML&topics=Web3)
):
    query_filters = {
        "$or": [
            {"isPrivate": False},
    ...
    if topics:
        # Filter rooms that have ALL selected topics
        # Use $all for "contains all of these" or $in for "contains any of these"
        # Based on "filtering on topics", $all seems more appropriate.
