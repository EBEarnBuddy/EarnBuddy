...
from pythonBackend.models.messages import MessageModel # Import MessageModel
from pythonBackend.models.pyobjectid import PyObjectId # For ID validation
from pythonBackend.dependencies import get_current_user_uid , ws_get_current_user_uid # For authentication
...
# Add this code block to backend/routes/messages.py


# New model for reaction data
...
async def add_reaction_to_message(
    ...
    # Find the message to verify it exists and the user has access
    existing_message = await db.messages.find_one({"_id": object_id})
    if not existing_message:
        raise HTTPException(status_code=404, detail="Message not found.")

    # Authorization: User must be a member of the room to add a reaction
    room_obj_id = existing_message.get("roomId")
    if room_obj_id:
        room = await db.rooms.find_one({"_id": room_obj_id, "members": current_user_uid})
        if not room:
            raise HTTPException(status_code=403, detail="Not authorized to react to messages in this room.")
    
    # Logic to update reactions
    # Use $addToSet to add the reaction to an array, ensuring uniqueness
    # Or, for more complex reaction counts, use a different structure
    update_result = await db.messages.update_one(
        {"_id": object_id},
        {"$addToSet": {"reactions": {"emoji": reaction.emoji, "userId": current_user_uid}}}
    )
