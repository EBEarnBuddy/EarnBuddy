...
const StartupsPage: React.FC = () => {
  ...
  const handleApplyToStartup = (startup: any, selectedRole?: any) => {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <AnimatePresence>
              {filteredStartups.map((startup, index) => {
                const hasApplied = currentUser ? (startup.applicants?.some(app => app.userId === currentUser.uid) || false) : false;
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mb-6">
                      <div className="flex items-center gap-1 text-sm text-gray-500 dark:text-gray-500">
                        ...
                        <span>{startup.totalApplicants || (startup.roles ? startup.roles.reduce((total, role) => total + (role.applicants?.length || 0), 0) : 0)} applicants</span>
                      </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        <h4 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">Open Positions:</h4>
                        <div className="space-y-2">
                          {startup.roles && startup.roles.length > 0 ? (
                            startup.roles.slice(0, 3).map((role) => (
                            <div key={role.id} className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
                              <div className="flex items-center justify-between mb-2">
                                <h5 className="font-medium text-gray-900 dark:text-white text-sm">{role.title}</h5>
                              ...
                              <motion.button
                                onClick={() => handleApplyToStartup(startup, role)}
                                disabled={hasApplied}
                                className={`w-full py-2 px-3 rounded-lg text-xs font-medium transition-all duration-300 ${
                                  hasApplied
                                    ...
                                    : 'bg-gradient-to-r from-emerald-600 to-emerald-500 text-white hover:shadow-md'
                                }`}
                          ...
                          {startup.roles && startup.roles.length > 3 && (
                            <div className="text-center">
                              <span className="text-sm text-gray-500 dark:text-gray-400">
                                +{startup.roles.length - 3} more positions
                              </span>
                            </div>
                          )}
                        </div>
                      ...
                      <div className="flex gap-3">
                        <motion.button
                          ...
                          whileTap={{ scale: hasApplied ? 1 : 0.98 }}
                        >
                          {hasApplied ? 'Applied' : 'View All Positions'}
                        </motion.button>
                        <motion.button
